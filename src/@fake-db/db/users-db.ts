			...
			groups: [
				{
					...
					name: 'Friends',
					userIds: ['5725a680bbcec3cc32a8488a', '5725a680e87cb319bd9bd673', '5725a6802d10e277a0f35775']
				},
				{
...
mock.onGet('/api/users').reply(config => {
	...
	switch (id) {
		case 'frequent': {
			response = usersDB.users.filter(user => usersDB.currentUser[0].frequentUsers.includes(user.id));
			break;
		}
		case 'starred': {
			response = usersDB.users.filter(user => usersDB.currentUser[0].starred.includes(user.id));
			break;
		}
		default: {
...
mock.onGet('/api/current-user').reply(config => {
	return [200, usersDB.currentUser[0]];
});

mock.onPost('/api/add-user').reply(request => {
...
mock.onPost('/api/update-user').reply(request => {
	const { user } = JSON.parse(request.data);

	usersDB.users = usersDB.users.map(_user => {
		if (user.id === _user.id) {
			return user;
		}
		return _user;
...
mock.onPost('/api/remove-user').reply(request => {
	const { userId } = JSON.parse(request.data);
	usersDB.users = usersDB.users.filter(user => userId !== user.id);

	return [200, userId];
});

mock.onPost('/api/remove-users').reply(request => {
	const { userIds } = JSON.parse(request.data);
	usersDB.users = usersDB.users.filter(user => !userIds.includes(user.id));
	return [200, userIds];
});

mock.onPost('/api/toggle-starred-user').reply(request => {
	const { userId } = JSON.parse(request.data);
	usersDB.currentUser[0].starred = _.xor(usersDB.currentUser[0].starred, [userId]);
	return [200, userId];
});

mock.onPost('/api/toggle-starred-users').reply(request => {
	const { userIds } = JSON.parse(request.data);
	usersDB.currentUser[0].starred = _.xor(usersDB.currentUser[0].starred, userIds);
	return [200, userIds];
});

mock.onPost('/api/set-users-starred').reply(request => {
	const { userIds } = JSON.parse(request.data);

	usersDB.currentUser[0].starred = [...usersDB.currentUser[0].starred, ...userIds];

	return [200, userIds];
});

mock.onPost('/api/set-users-unstarred').reply(request => {
	const { userIds } = JSON.parse(request.data);

	usersDB.currentUser[0].starred = usersDB.currentUser[0].starred.filter(
